...
ad_form -name vetrina \
	...
	db_1row query "select telefono, fax, indirizzo, comune_id as citta, sitoweb, cellulare, facebook, twitter, linkedin, gplus, societa_id, citazione, esperienza, clienti, patrimonio, attivita, esperienze, formazione from pe_professionisti where professionista_id = :professionista_id"
	...
	#Immagine sfondo
	if {[string match *.jpg [string tolower $upload_sfondo]] || [string match *png* [string tolower $upload_sfondo]] || [string match *.jpeg [string tolower $upload_sfondo]]} {
	    #Existing file for upload
	    db_transaction {
		regsub -all {\\} $upload_sfondo "<" upload_sfondo
		set filename $permalink
		if {[string match *.jpg $upload_sfondo] || [string match *.JPG $upload_sfondo] || [string match *.jpeg $upload_sfondo] || [string match *.JPEG $upload_sfondo]} {
		    append filename ".jpg"
		}
		if {[string match *.png $upload_sfondo] || [string match *.PNG $upload_sfondo]} {
		    append filename ".png"
		}
		ns_rename $filepath /usr/share/openacs/packages/patrimoniaexpert/www/images/professionisti_backgrounds/$filename
		db_dml query "UPDATE pe_professionisti set sfondo = :filename where professionista_id = :professionista_id"
	    }
	}
	#Inserimento dati in pe.professionisti
	db_dml query "update pe_professionisti set telefono = :telefono, fax = :fax, indirizzo = :indirizzo, comune_id = :citta, sitoweb = :sitoweb, facebook = :facebook, twitter = :twitter, linkedin = :linkedin, gplus = :google, citazione = :citazione, esperienza = :esperienza, clienti = :clienti, patrimonio = :patrimonio_gestito, attivita = :attivita, esperienze = :esperienze, formazione = :formazione where professionista_id = :professionista_id"
